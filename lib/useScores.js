...
export function useTodayScores() {
  ...
  useEffect(() => {
    ...
    function settle() { if (pending > 0) { pending--; if (pending === 0) setLoading(false); } }
    // Without an error callback, a listener that fails (permissions,
    // network) never calls back at all — scores[gameId] stays undefined
    // forever and the box is stuck showing "…" with no way out.
    const unsubs = GAME_IDS.map(gameId => onSnapshot(
      doc(db, 'scores', uid, gameId, today),
      snap => { setScores(prev => ({ ...prev, [gameId]: snap.exists() ? snap.data() : null })); settle(); },
      err => { console.error(`Couldn't load today's ${gameId} score:`, err); setError(err); settle(); },
    ));
    return () => unsubs.forEach(u => u());
  }, [uid, retryTick]);

  return { scores, loading, error, retry: () => setRetryTick(t => t + 1) };
}
...
function subscribePlayerGame(uid, gameId, onChange) {
  ...
  if (!entry) {
    ...
    entry.unsubscribe = onSnapshot(collection(db, 'scores', uid, gameId), snap => {
      ...
      // Without this, a failed listener here leaves every dependent view
      // (tournament standings, personal history, daily-compare) silently
      // stuck on its loading state forever, with nothing in the console to
      // explain why — same failure mode as useTodayScores above.
      console.error(`Couldn't load ${gameId} history for ${uid}:`, err);
    });

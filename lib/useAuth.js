...
export function AuthProvider({ children }) {
  ...
  // Completes onboarding: claims a username and records which games this user
  // plays. Atomic via transaction — the usernames/{lowercased} doc doubles as
  // the uniqueness lock, enforced again server-side by firestore.rules.
  async function completeOnboarding({ username, gamesPlayed }) {
  ...
  async function updateGamesPlayed(gamesPlayed) {
    if (!user?.uid) throw new Error('You must be signed in.');
    const userRef = doc(db, 'users', user.uid);
    await runTransaction(db, async (tx) => {
      tx.set(userRef, { gamesPlayed }, { merge: true });
    });
    setProfile(prev => (prev ? { ...prev, gamesPlayed } : prev));
  }
  ...
  return (
    <AuthContext.Provider value={{
      ...
      completeOnboarding, updateGamesPlayed,
    }}>
      {children}

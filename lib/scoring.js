...
// A member who joined mid-period shouldn't have days before they joined
// counted against (or for) them — scores are global, so someone can easily
// have logged times for a game before they ever joined THIS tournament.
// joinedAtByUid (optional, { [uid]: 'YYYY-MM-DD' }) pushes their effective
// window start forward to their join date when it's later than the period.
...
function inPeriodForUid(entries, periodStart, periodEnd, uid, joinedAtByUid) {
  return inPeriod(entries, effectiveStart(uid, periodStart, joinedAtByUid), periodEnd);
}

// ── Eligibility (shared by all four metrics) ──────────────────────────────
// A participant's personal window runs from their first submission in the
// period (never earlier than their tournament join date — see
// effectiveStart above) to the period's latest submission across everyone
// (capped at periodEnd). They qualify if they submitted on at least
// thresholdPct% of the days in that window — mirrors
// getTournamentParticipants() exactly, generalized off the fixed
// 50%/calendar-month original.
export function getEligibleParticipants(entriesByUid, periodStart, periodEnd, thresholdPct, joinedAtByUid) {
  let latestDate = null;
  for (const [uid, entries] of Object.entries(entriesByUid)) {
    for (const e of inPeriodForUid(entries, periodStart, periodEnd, uid, joinedAtByUid)) {
  ...
  const result = [];
  for (const [uid, entries] of Object.entries(entriesByUid)) {
    const mine = inPeriodForUid(entries, periodStart, periodEnd, uid, joinedAtByUid);
...
// participants *who had already joined by that day* who submitted, fastest
// first; points = max(N - rank, minPts) with ties sharing the same
// rank/points; minPts is 2 if anyone active that day was missing (else 1),
// so a slow-but-present day still beats an absent one; non-submitters
// explicitly score 0 that day. N is that day's active roster size — a
// member who joins partway through the period doesn't inflate (or get
// penalized against) days before they were around to compete on.
export function computeSimpleScores(entriesByUid, periodStart, periodEnd, thresholdPct, joinedAtByUid) {
  const participants = getEligibleParticipants(entriesByUid, periodStart, periodEnd, thresholdPct, joinedAtByUid);
  if (participants.length === 0) return { participants: [], totals: {}, daily: [], sortDir: 'desc' };

  const dayMap = {}; // date -> uid -> best timeSeconds
  for (const uid of participants) {
    for (const e of inPeriodForUid(entriesByUid[uid] || [], periodStart, periodEnd, uid, joinedAtByUid)) {
  ...
  for (const date of Object.keys(dayMap).sort()) {
    ...
    const active = participants.filter(p => date >= effectiveStart(p, periodStart, joinedAtByUid));
    const N = active.length;
    const submitters = active.filter(p => uidTime[p] != null)
      .sort((a, b) => uidTime[a] - uidTime[b]);
    const missing = active.filter(p => uidTime[p] == null);
    ...
    active.forEach(p => { totals[p] += (dayPts[p] || 0); });
    daily.push({ date, dayPts, submitters, missing, N });
  }

  return { participants, totals, daily, sortDir: 'desc' };
}
...
export function computeRatioScores(entriesByUid, periodStart, periodEnd, thresholdPct, mode = 'ratio', joinedAtByUid) {
  const participants = getEligibleParticipants(entriesByUid, periodStart, periodEnd, thresholdPct, joinedAtByUid);
...
// ── Dispatcher ─────────────────────────────────────────────────────────────
export function computeStandings(metric, entriesByUid, periodStart, periodEnd, thresholdPct, joinedAtByUid) {
  if (metric === 'simple') return computeSimpleScores(entriesByUid, periodStart, periodEnd, thresholdPct, joinedAtByUid);
  return computeRatioScores(entriesByUid, periodStart, periodEnd, thresholdPct, metric, joinedAtByUid);
}

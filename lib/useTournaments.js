...
import { DEFAULT_REVEAL_POLICY, DEFAULT_REVEAL_INTERVAL_DAYS } from './reveal';
...
export function useTournaments() {
  ...
  const createTournament = useCallback(async ({
    name, games, scoringMetric, eligibilityThresholdPct, startDate, joinPolicy, inviteUids = [],
    revealPolicy = DEFAULT_REVEAL_POLICY, revealIntervalDays = DEFAULT_REVEAL_INTERVAL_DAYS,
  }) => {
...
// Personal, per-user display preferences for a tournament — pinning it to
// the top of the home page list, and giving its card a color of the
// viewer's own choosing. Deliberately not tournament-wide settings (no
// owner gate, no shared state): everyone in a tournament can pin/color it
// on their own home screen independently of what anyone else does.
export function useTournamentPrefs() {
  const { user } = useAuth();
  const uid = user?.uid;
  const [prefs, setPrefs] = useState({}); // tournamentId -> { pinned, color }

  useEffect(() => {
    if (!uid) { setPrefs({}); return; }
    return onSnapshot(collection(db, 'users', uid, 'tournamentPrefs'), snap => {
      const map = {};
  ...
  const setPinned = useCallback(async (tournamentId, pinned) => {
    if (!uid) return;
    await setDoc(doc(db, 'users', uid, 'tournamentPrefs', tournamentId), { pinned }, { merge: true });
  }, [uid]);

  const setColor = useCallback(async (tournamentId, color) => {
    if (!uid) return;
    await setDoc(doc(db, 'users', uid, 'tournamentPrefs', tournamentId), { color }, { merge: true });
  }, [uid]);

  return { prefs, setPinned, setColor };
}
...
// Live join-date-per-member map, keyed by uid -> 'YYYY-MM-DD' (the local
// date each members/{uid} doc's joinedAt resolves to). Threaded through
// every standings computation (lib/scoring.js's effectiveStart()) so a
// member who joins mid-tournament doesn't have days before they joined
// counted against — or credited to whoever they'd have "beaten" — them. A
// member whose joinedAt hasn't resolved yet (serverTimestamp() briefly
// reads null right after the write) is simply omitted, which scoring.js
// treats as "no constraint" until the next snapshot fills it in.
export function useTournamentMemberJoinDates(tournamentId) {
  const [joinedAtByUid, setJoinedAtByUid] = useState({});

  useEffect(() => {
    if (!tournamentId) { setJoinedAtByUid({}); return; }
    return onSnapshot(collection(db, 'tournaments', tournamentId, 'members'), snap => {
      const map = {};
      snap.docs.forEach(d => {
        const ts = d.data().joinedAt;
        if (ts?.toDate) {
          const dt = ts.toDate();
          map[d.id] = `${dt.getFullYear()}-${String(dt.getMonth() + 1).padStart(2, '0')}-${String(dt.getDate()).padStart(2, '0')}`;
        }
      });
      setJoinedAtByUid(map);
    });
  }, [tournamentId]);

  return joinedAtByUid;
}
...
export async function updateTournamentSettings(id, {
  name, games, scoringMetric, eligibilityThresholdPct, startDate, joinPolicy,
  revealPolicy = DEFAULT_REVEAL_POLICY, revealIntervalDays = DEFAULT_REVEAL_INTERVAL_DAYS,
}) {
  const trimmedName = name.trim();

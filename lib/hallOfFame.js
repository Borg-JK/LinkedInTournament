...
export function computeMonthWinnerForGame(entriesByUid, monthKey, tournament, periodEnd) {
...
export function computeMonthWinnerOverall(entriesByUidByGame, monthKey, tournament, periodEnd) {
  const { start, end } = monthBounds(monthKey, tournament.startDate, periodEnd);
  if (start > end) return null;
  ...
  const { ranked } = computeTournamentStandings(monthTournament, entriesByUidByGame, end);
  if (ranked.length === 0) return null;
  const top = ranked[0];
